...
\usepackage{float}
\usepackage{booktabs}
\usepackage{siunitx}   % for decimal alignment of metrics
\usepackage{tabularx}
\usepackage{color}
...
\lstdefinestyle{verbwrap}{
  basicstyle=\ttfamily\small,   % monospace, verbatim-like
  breaklines=true,              % wrap long lines
  breakatwhitespace=false,      % CRUCIAL: break anywhere, even mid-token
  breakindent=0pt,              % no indent on wrapped continuation
  postbreak=\mbox{\textcolor{gray}{$\hookrightarrow$}\space}, % continuation marker
  columns=fixed,                % keep the monospace grid (true verbatim look)
  keepspaces=true,              % preserve runs of spaces
  showstringspaces=false,
  upquote=true,                 % straight quotes, not curly
  literate={`}{{`}}1,           % keep backtick literal
  frame=none,                   % no box; use single if you want a rule
  xleftmargin=0pt,
  numbers=none,
}

% make it the default for all lstlistings:
\lstset{style=verbwrap}
...
The preprocessing pipeline differs between the regression and classification tasks. For regression, no additional filtering is applied as sliding windows are generated with a minimum length of 3 and a maximum length of 100 events per prefix, producing truncated event histories that capture recent temporal context. For classification, additional constraints ensure label consistency and prevent information leakage: cases without any defined outcome events are removed (98 out of 31,509 cases), and windows containing an outcome event are excluded since their inclusion would leak information about the final case outcome into the input representation. The same minimum and maximum window lengths (3 to 100 events) are used as in the regression setup.
...
prefixes from the same case remain in the same split. Cases ending before the 0.8
quantile go to training while cases starting later go to testing, simulating an
operational setting where the model predicts on cases that start after the
training period. Cases that fall into neither set are dropped. \autoref{tab:split-stats} shows the resulting split
statistics.
...
\begin{table}[t]
    ...
    \begin{tabular}{lrr}
        ...
        Split timestamp & \multicolumn{2}{c}{2016-10-18 11:39:41} \\
        \midrule
        \multicolumn{3}{l}{\textbf{Regression task}} \\
        Cases           & 22{,}895  & 6{,}302 \\
        Prefixes        & 827{,}614 & 228{,}455 \\
        Split timestamp & \multicolumn{2}{c}{2016-10-18 22:39:59} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\paragraph{Feature Importance}
Feature Importance is computed the same way as for classification (see \autoref{sec:results-outcome}).
...
\begin{figure}[H]
    ...
    \caption{Train vs.\ Test performance (all models log-based + time-series) for classification.}
...
and evaluation work proceeded somewhat in parallel, so some inconsistencies surfaced only when the full comparison was assembled.
...
drift check before modelling. We would add recent and seasonal lags (t-1h, t-24h, t-168h)
and detrended workload signals given the strong daily/weekly priors and the
observed non-stationarity. We would perform feature selection on a dedicated validation
split rather than the final test set. We would profile runtime from the start rather than
at the end. We would add a smoke check that every configured feature actually fires
