...
between train and test on the chronological split, so they fail to generalise. We also
investigated proxy effects, extraction-template limitations, and the curse of
dimensionality. The discovery of the \texttt{WaitingStateFeatures} activity-matching
bug, where matching on \texttt{O\_Sent} silently never fired because the real event
names include parenthetical suffixes, occurred during this phase and needed to be
documented as a limitation. Right-censoring at the end of the log surfaced only during
long-prefix analysis. All findings went into the evaluation discussion.

\subsection{Task Tracking}
We managed all work through GitHub Issues with linked pull requests. The requirements
tracking system kept detailed sub-requirements as GitHub issues linked
to parent requirements instead of editing the requirements document, which worked well
for scope changes like adding HGB as a model family.
