...
Four model families were compared for each task (\autoref{tab:models}),
including tree-based ensemble methods and linear baselines that handle
non-linear relationships without requiring feature scaling. All
classification models use \texttt{class\_weight="balanced"} to address class imbalance.
...
Hyperparameters are optimised via \texttt{RandomizedSearchCV} with
5-fold cross-validation using all training prefixes for faster
iteration.
